...
Photobooth = function(e) {
    ...
    var t=navigator.mediaDevices.getUserMedia(constraints);
	this.onImage = function() {}, this.pause = function() {
		f === !1 && (f = !0, l && l.stop && l.stop())
	}, this.resume = function() {
		f === !0 && (f = !1, A())
	}, this.destroy = function() {
		this.pause(), e.removeChild(g)
		...
		h = e, p = t, T.setMax(h, p), g.style.width = e + "px", g.style.height = t + "px", y.width = e, y.height = t, w.width = e, w.height = t, S.width = e, S.height = t
	};
	var n = function(e) {
	...
	Drag = function(e) {
		var t = 0,
			n = 0,
			r = this;
		this.onStart = function() {}, this.onMove = function() {}, this.onStop = function() {};
		var i = function(e) {
				e.preventDefault(), t = e.clientX, n = e.clientY, r.onStart(t, n), document.addEventListener("mousemove", s, !1), document.addEventListener("mouseup", o, !1)
			},
			s = function(e) {
				r.onMove(e.clientX - t, e.clientY - n)
			},
			o = function(e) {
				r.onStop(e.clientX - t, e.clientY - n), document.removeEventListener("mousemove", s), document.removeEventListener("mouseup", o)
			};
	...
	var r = function(e, t) {
			...
			var r = 50,
				i = 50,
				s = e.getElementsByClassName("handle")[0],
				o = e.getElementsByClassName("slider")[0],
				u = new Drag(s);
			u.onMove = function(e) {
				a(r + e)
			}, u.onStop = function(e) {
			...
			var a = function(e) {
					e > 0 && e < 100 && (i = e, s.style.left = e + "px", t((e - 50) / 100))
				},
				f = function(e) {
					a(e.layerX), r = i
				};
			o.addEventListener("click", f, !1), s.addEventListener("click", n, !1)
		},
		i = function(e, t, r) {
			this.setMax = function(e, n) {
				...
				h === !1 ? (p.style.opacity = 1, h = !0) : (p.style.opacity = 0, h = !1)
			};
			var i = 30,
				s = 30,
				o = 100,
			...
			p.className = "resizehandle", p.innerHTML = '<div class="handle"><div></div></div>', e.appendChild(p);
			var d = p.getElementsByTagName("div")[0],
				v = new Drag(d);
			v.onMove = function(e, n) {
				i + e + o < t && i + e > 0 && (a = i + e, p.style.left = a + "px"), s + n + u < r && s + n > 0 && (f = s + n, p.style.top = f + "px")
			}, v.onStop = function() {
				i = a, s = f
			};
			var m = p.getElementsByTagName("div")[1];
			m.addEventListener("mousedown", n, !1);
			var g = new Drag(m);
			g.onMove = function(e, n) {
				i + e + o < t && o + e > 18 && (l = o + e, p.style.width = l + "px"), s + n + u < r && u + n > 18 && (c = u + n, p.style.height = c + "px")
			}, g.onStop = function() {
				o = l, u = c
			}
		},
		...
		v = function(e) {
			return g.getElementsByClassName(e)[0]
		},
		m = function(e) {
			return document.createElement(e)
		},
		g = m("div");
	g.className = "photobooth", g.innerHTML = '<div class="blind"></div><canvas></canvas><div class="warning notSupported">Sorry, Photobooth.js is not supported by your browser</div><div class="warning noWebcam">Please give Photobooth permission to use your Webcam. <span>Try again</span></div><ul><li title="hue"class="hue"></li><li title="saturation"class="saturation"></li><li title="brightness"class="brightness"></li><li title="crop"class="crop"></li><li title="take picture"class="trigger"></li></ul>';
	var y = m("canvas"),
		b = y.getContext("2d"),
		w = g.getElementsByTagName("canvas")[0],
		E = w.getContext("2d"),
		S = m("video");
	S.autoplay = !0;
	var x = v("noWebcam");
	x.getElementsByTagName("span")[0].onclick = function() {
		A()
	}, new r(v("hue"), function(e) {
		s = e
	}), new r(v("saturation"), function(e) {
		o = e
	}), new r(v("brightness"), function(e) {
		u = e
	});
	var T = new i(g, h, p),
		N = v("crop");
	N.onclick = function() {
		T.toggle(), N.className === "crop" ? N.className = "crop selected" : N.className = "crop"
	};
	var C = v("blind");
	v("trigger")
		.onclick = function() {
			C.className = "blind", C.style.opacity = 1, setTimeout(function() {
				C.className = "blind anim", C.style.opacity = 0
			}, 50);
			var e = {};
			T.isActive() ? e = T.getData() : a ? e = {
				x: (h - S.videoWidth) / 2,
				y: (p - S.videoHeight) / 2,
				width: S.videoWidth,
			...
			if (a) t.getContext("2d")
				.drawImage(S, Math.max(0, e.x - (h - S.videoWidth) / 2), Math.max(e.y - (p - S.videoHeight) / 2), e.width, e.height, 0, 0, e.width, e.height);
			else {
				var n = E.getImageData(e.x, e.y, e.width, e.height);
				t.getContext("2d")
					.putImageData(n, 0, 0)
			}
			c.onImage(t.toDataURL())
		};
	var k = function(e) {
			l = e;
			try {
                //Edit by wellhi 2023年6月20日
                //S.show();
                S.srcObject = l, d(D);
				//S.src = (window.URL || window.webkitURL)
				//	.createObjectURL(l), d(D)
			} catch (t) {
				S.mozSrcObject = l, c.forceHSB === !1 ? (a = !0, g.appendChild(S), g.getElementsByTagName("ul")[0].className = "noHSB") : S.addEventListener("canplay", function() {
					d(D)
				}, !1), S.play()
			}
		},
		...
		A = function() {
			x.style.display = "none", t.call(navigator, {
				video: !0
			}, k, L)
		},
		O = function(e, t, n) {
		...
		D = function() {
			b.drawImage(S, 0, 0, h, p);
			var e = b.getImageData(0, 0, h, p),
				t = e.data;
			for (var n = 0; n < t.length; n += 4) {
				...
				if (l == c) v = m = 0;
					...
					var y = l - c;
					m = g > .5 ? y / (2 - l - c) : y / (l + c), l === r && (v = ((i - a) / y + (i < a ? 6 : 0)) / 6), l === i && (v = ((a - r) / y + 2) / 6), l === a && (v = ((r - i) / y + 4) / 6)
				}
				v = M(v + s), m = _(m + o), g = _(g + u);
				if (m === 0) r = i = a = g;
				else {
					var w = g < .5 ? g * (1 + m) : g + m - g * m,
						x = 2 * g - w;
					r = O(x, w, v + 1 / 3), i = O(x, w, v), a = O(x, w, v - 1 / 3)
				}
				t[n] = r * 255, t[n + 1] = i * 255, t[n + 2] = a * 255
			}
			E.putImageData(e, 0, 0), f === !1 && d(D)
		};
	this.resize(h, p), e.appendChild(g), t ? d(A) : v("notSupported")
		.style.display = "block"
}
/**
